	...
	try {
		...
		if (existingBet) {
			db.update(schema.bets)
				.set({ amount: existingBet.amount + amount })
				.where(eq(schema.bets.id, +existingBet.id));
		} else {
		...
		await db
			.update(schema.wetlandWagers)
			.set({ breadcrumbsWallet: wetlanderwager.breadcrumbsWallet - amount })
			.where(eq(schema.wetlandWagers.id, +wetlanderwager.id));

		return c.text(
			`${wetlanderwager.name}has made a bet , wallet: ${[wetlanderwager.breadcrumbsWallet - amount]} breadcrumbs`,
		);
	} catch (e) {
		console.error(e);
...
wetlandwagers.get("/:id/wallet", async (c) => {
	...
	return c.text(
		`${wetlanderwager.name}'s wallet contains: ${wetlanderwager.breadcrumbsWallet} breadcrumbs`,
	);
});
...
wetlandwagers.post("/:id/cheat", async (c) => {
	...
	const { performanceindicator } = (await c.req.json()) as {
		performanceindicator: keyof typeof schema.geese;
	};
	if (
		...
		return c.text(
			"Sorry, this performance indicator doesn't exist. Valid training focuses are speed, style, precision, or efficiency.",
			404,
		);
	}
	...
	try {
		...
		if (randomNumber[0] < successThreashold) {
			db.update(schema.wetlandWagers)
				...
				.where(eq(schema.wetlandWagers.id, +wetlanderwager.id));
			return c.text(
				"Your cunning scheme ruffled a few feathers! You've been caught and now face a 10,000 breadcrumbs penalty.",
				403,
			);
		}
		db.update(schema.geese)
			.set({ [performanceindicator]: goose?.[performanceindicator] + 5 })
			.where(eq(schema.geese.id, +gooseId));
		return c.text(
			"With a sly grin and a flick of a feather, you outsmarted the flock—no one at the Gooselympics suspected a thing!",
			200,
		);
	} catch (e) {
		console.error(e);
...
wetlandwagers.post("/:id/bar", async (c) => {
	...
	try {
		...
		db.update(schema.wetlandWagers)
			.set({ breadcrumbsWallet: wetlanderwager.breadcrumbsWallet - 100 })
			.where(eq(schema.wetlandWagers.id, +wetlanderwager.id));
		db.update(schema.geese)
			.set({ energyLevel: goose.energyLevel + 5 })
			.where(eq(schema.geese.id, +goose.id));

		return c.text(
			`${wetlanderwager.name} bought a ${goose.favouriteCocktail} for ${goose.name}`,
			200,
		);
	} catch (e) {
		console.error(e);
