...
geese.post("/:id/train", async (c) => {
	...
	if (energyLevel < 1) {
		return c.text(
			"Sorry the goose has not enough energy left for training",
			403,
		);
	}
	...
	try {
		...
		await db
			.update(schema.geese)
			.set({
				energyLevel: goose.energyLevel - 3,
				[performanceindicator]: goose?.[performanceindicator] + 0.5,
			})
			.where(eq(schema.geese.id, +goose.id));
		return c.text(
			`Training done, new energy level is  ${newEngeryLevel} New ${[performanceindicator]} value: ${newLevel}`,
		);
	} catch (e) {
		console.error(e);
...
geese.post("/:id/speed-challenge", async (c) => {
	...
	try {
		const [opponentGoose] = await db
			.select()
			.from(schema.geese)
			.where(eq(schema.geese.id, +opponentId));
		if (!opponentGoose) {
		...
		await db
			.update(schema.geese)
			.set({ energyLevel: newEnergyLevel, speed: newSpeedLevel })
			.where(eq(schema.geese.id, +challengerGoose.id));
