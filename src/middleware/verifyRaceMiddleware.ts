...
export const verifyRaceMiddleware = createMiddleware(async (c, next) => {
  ...
  const raceDb = (await db.select().from(schema.races).where(eq(schema.races.id, + id)).limit(1))[0];
  ...
  const race: Race = {
    id: raceDb.id,
    name: raceDb.name,
    type: raceDb.type && Object.values(raceTypesEnum).includes(raceDb.type as raceTypesEnum)
...
for (const value of arr_geeseId) {
  ...
  const goose = (await db.select().from(schema.geese).where(eq(schema.geese.id, gooseId)).limit(1))[0];
